...
class turtleCommandsClass():
	...
	#Getting Information
	intCommandsFull=pickle.load(open('intCommandsFull.txt','rb'))
	intCommandsShort=pickle.load(open('intCommandsShort.txt','rb'))
	nonValueCommands=pickle.load(open('nonValueCommands.txt','rb'))
	arduinoCode = pickle.load(open('arduinoCode.txt','rb'))
	...
	def saveArduinoCode(self, fileName='arduino Code.ino'):
...
def main():
	s='fd 50 rt 90'				#input('Enter the Turtle Command')
